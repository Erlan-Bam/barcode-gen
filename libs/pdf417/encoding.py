...
def encode(
    ...
    if security_level < 0 or security_level > 8:
        raise ValueError(
            ("'security_level' must be " "between 1 and 8. Given: %r") % security_level
        )
    ...
    code_words = encode_high(
        config, data_bytes, num_cols, security_level, numeric_compaction
    )

    rows = list(chunks(code_words, num_cols))

    return list(encode_rows(config, rows, num_cols, security_level))


def encode_rows(config, rows, num_cols, security_level):
    num_rows = len(rows)
   

    for row_no in range(num_rows):
        row_data = rows[row_no]
...
def encode_high(config, data, columns, security_level, numeric_compaction=True):
    ...
    data_words = list(compact(config, data, numeric_compaction))

    data_count = len(data_words)
    # Get the padding to align data to column count
    ec_count = 2 ** (security_level + 1)

    padding_words = get_padding(data_count, ec_count, columns, add_rows)
    ...
    try:
        add_rows = int(config["minRows"]) - int(
            len(extendend_words + ec_words) / columns
        )
        if add_rows < 0:
            add_rows = 0
    except:
        pass
    try:
        global PADDING_CODE_WORD
        PADDING_CODE_WORD = int(config["paddingWord"])
    except:
        pass
...
def get_padding(data_count, ec_count, num_cols, add_rows):
    ...
    return (
        [PADDING_CODE_WORD] * ((num_cols - mod) + (add_rows * num_cols))
        if mod > 0
        else (
            [PADDING_CODE_WORD]
            * ((num_cols - (mod if mod else num_cols)) + (add_rows * num_cols))
            if add_rows
            else []
        )
    )
